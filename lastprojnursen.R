...
summary(met000) # graph of maximum average and min temperatures for given day in march 2019
met000%>%
...
aksaraymet %>% left_join(translatemonth)%>%
  ...
  layout(
    title = "Kış ayı ortalama sıcaklıkları",
    xaxis = list(title = "Yıllar",
                 tickfont = f1)
  )
bardol <- read_excel('istanbul-baraj-doluluk-oranlar-tr-en.xlsx',3)
summary(bardol)
bardol%>%
  ggplot(aes(x = Tarih, y = `Doluluk Oranı (%)` )) + 
  geom_point() + geom_line()+geom_smooth()+
  labs(title="Baraj Doluluk oranları")
temizsu <- read_excel('istanbula-verilen-temiz-su-miktarlar-tr-en.xlsx',3)
temizsu%>%
  gather(key = yıl, value = miktar,-Ay)%>% 
  left_join(translatemonth)

%>%
  unite(datetime, yıl, Month,1)
library(kableExtra)
windstations <- read_excel('rüzgarsantralleri.xlsx',1)
windstations %>%
  kable() %>%
  kable_styling()
windstations <- read_excel('rüzgarsantralleri.xlsx',1)
knitr::kable(windstations)
